...
#    define CLOCK_START_OFFSET (LDCP_S2NS(3600 * 24))
...
#    elif defined(HAVE_CLOCK_GETTIME)
    struct timespec tm;
    ldcp_assert(clock_gettime(CLOCK_MONOTONIC, &tm) != -1);
...
#    elif defined(HAVE_QUERYPERFORMANCECOUNTER)
    ...
    if (pf.QuadPart == 0) {
        ldcp_assert(QueryPerformanceFrequency(&pf));
        ldcp_assert(pf.QuadPart != 0);
